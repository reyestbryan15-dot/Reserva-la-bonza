...
// IMPORTAMOS TU LOGO DESCARGADO
// Asegúrate de que el archivo esté en src/assets/bancolombia.svg
import bancolombiaLogo from '../assets/bancolombia.svg';

// Icono SVG de WhatsApp (inline para no cargar imágenes extra)
const WhatsAppIcon = () => (
...
const PaymentCard = ({ 
  ...
  const handleCopy = () => {
    navigator.clipboard.writeText(bancolombiaNumber);
    setCopied(true);
  ...
  return (
    <div className="max-w-md mx-auto my-8 relative animate-in fade-in zoom-in duration-500">
      ...
      {/* 1. CABECERA (Amarillo Bancolombia) */}
      <div className="bg-[#FDDA24] text-black p-6 rounded-t-3xl shadow-lg relative overflow-hidden">
        <div className="flex justify-between items-end z-10 relative">
            ...
            {/* AQUÍ USAMOS TU IMAGEN SVG LOCAL */}
            <div className="bg-white/90 px-4 py-2 rounded-lg shadow-sm border border-white/50 backdrop-blur-sm">
                <img 
                  src={bancolombiaLogo} 
                  alt="Bancolombia" 
                  className="h-8 object-contain" // Ajusta h-8 a h-10 si lo quieres más grande
                />
            </div>
        </div>
