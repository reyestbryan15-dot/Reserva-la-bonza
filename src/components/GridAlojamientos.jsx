...
const GridAlojamientos = ({ limit }) => {
  ...
  const filtroTipo = searchParams.get('tipo') || 'todo';
  ...
  useEffect(() => {
    ...
    fetchAllData();
  }, [limit, filtroDestino, filtroTipo, filtroGuests]);
  ...
  if (filtroTipo === 'venta' && !limit) tituloPagina = t('search.for_sale');
  if (filtroTipo === 'alquiler' && !limit) tituloPagina = t('search.rent_only');

  return (
    <div className="max-w-7xl mx-auto px-4 py-12">
      {!limit && (
        <div className="mb-8">
          <Link to="/" className="group inline-flex items-center gap-3 px-5 py-2.5 rounded-2xl bg-white border-2 border-gray-100 shadow-sm hover:shadow-md hover:border-indigo-200 hover:bg-indigo-50/50 transition-all duration-300">
      ...
      {properties.length === 0 ? (
        <div className="flex flex-col items-center justify-center py-20 text-gray-400 bg-gray-50 rounded-[3rem] border-2 border-dashed border-gray-200">
          ...
          <p className="text-xl font-medium">{t('lodging.no_results_in')}</p>
      ...
      {limit && (
        <div className="text-center">
          <Link to="/propiedades" className="inline-block bg-gradient-to-r from-indigo-600 to-indigo-800 text-white px-10 py-4 rounded-full font-bold hover:scale-105 transition shadow-xl">
            {t('lodging.view_all')}
          </Link>
        </div>
      )}
